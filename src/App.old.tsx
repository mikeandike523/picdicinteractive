...
const DropZone: React.FC<DropZoneProps> = ({
  ...
  const xFraction = x / imageWidth;
  const yFraction = y / imageHeight;
  const widthFraction = width / imageWidth;
  const heightFraction = height / imageHeight;

  const formatPercent2Decs = (fraction: number) => (fraction*100).toFixed(2) + "%";
  ...
  const style: React.CSSProperties = {
    position: "absolute",
    left: formatPercent2Decs(xFraction),
    top: formatPercent2Decs(yFraction),
    width: formatPercent2Decs(widthFraction),
    height: formatPercent2Decs(heightFraction),
    border: borderStyle,
    backgroundColor,
    display: "flex",
    alignItems: "center",
...
const App: React.FC = () => {
  ...
  const wordList = pageData.annotations.map((ann, idx) => ({
    id: idx,
    word: ann.text,
  }));
